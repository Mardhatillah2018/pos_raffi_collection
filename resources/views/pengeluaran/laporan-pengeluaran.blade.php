...
<body>
  ...
  <!-- Halaman 1: Ringkasan per kategori -->
  <header>
    <h2>Laporan Pengeluaran (Ringkasan Per Kategori)</h2>
  ...
  <table>
    ...
    <tbody>
      ...
      @foreach($ringkasanKategori as $index => $data)
        <tr>
          <td>{{ $index + 1 }}</td>
          <td>{{ $data->kategori->nama_kategori ?? '-' }}</td>
          <td class="text-right">Rp {{ number_format($data->total_per_kategori, 0, ',', '.') }}</td>
        </tr>
        @php $grandTotal += $data->total_per_kategori; @endphp
      @endforeach
      <tr class="total-row">
  ...
  <header>
    <h2>Laporan Pengeluaran (Detail)</h2>
    <h4 style="margin-bottom: 6px">{{ $namaCabang }}</h4>
    <div style="border-bottom: 2px solid #000; width: 100%; margin: 0 auto 10px;"></div>
    <div class="tanggal-cetak">
        Tanggal Cetak: {{ \Carbon\Carbon::parse($tanggalCetak)->format('d M Y') }}
  ...
  <table>
    ...
    <tbody>
      ...
      @foreach($pengeluarans as $index => $pengeluaran)
        ...
        @php $totalDetail += $pengeluaran->total_pengeluaran; @endphp
      @endforeach
      <tr class="total-row">
        <td colspan="4" class="text-right">Total</td>
        <td class="text-right">Rp {{ number_format($totalDetail, 0, ',', '.') }}</td>
      </tr>
    </tbody>
  </table>
  @else
